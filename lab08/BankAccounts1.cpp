...
BankAccount *BankAccount_new(string owner, int accountNum, 
        ...
        account_ptr->balance       = balanceDollars + 0.01 * balanceCents;
        account_ptr->interestRate  = interestRate;
 ...
 */
float BankAccount_balance(BankAccount *accountp) {
        return accountp == nullptr ? -1 : accountp->balance;
}
...
void BankAccount_deposit(BankAccount *accountp, int dollars, int cents) {
        if (accountp != nullptr) {
        float change = dollars + 0.01 * cents; 
        
        accountp->balance += change;
    }
}
